...
export const LIFE_PROPS=[...LIFE_PLACES.map(p=>({...p,action:p.id==='clubdesk'?'life-club':'life-homes'})),...HOMES.map(h=>({id:`home-${h.id}`,name:`${h.number} · ${h.name}`,x:h.doorX+.25,z:h.doorZ,y:h.floorY??4.4,action:'life-homes'}))].map(p=>({...p,type:'prop',aimY:1.4,reach:2.8,prompt:`[E] ${p.name.toUpperCase()}`}));
export function buildLifeWorld(scene,colliders){
 ...
 for(const home of HOMES){const r=HOME_BOUNDS.find(r=>r.id===home.id),room=new THREE.Group(),color=mat(home.color),cx=(r.minX+r.maxX)/2,cz=(r.minZ+r.maxZ)/2;
  room.position.y=(home.floorY??4.4)-4.4;room.name=home.name;root.add(room);
  ...
  if(home.floorY===0){
   // Park homes have a separate living area and compact kitchen, clear of the door.
   box(room,color,cx,4.82,r.maxZ-1.1,3,.6,1);
   box(room,color,cx,5.3,r.maxZ-.65,3,.65,.16);
   for(const side of [-1,1])box(room,color,cx+side*1.42,5.15,r.maxZ-1.1,.18,.65,1);
   box(room,wood,cx,4.98,r.maxZ-2.8,1.6,.1,.8);
   for(const dx of [-.65,.65])box(room,brass,cx+dx,4.7,r.maxZ-2.8,.06,.55,.06);
   box(room,wood,r.minX+.5,4.94,cz,1,1.05,2.5);
   box(room,white,r.minX+.5,5.5,cz,1.1,.1,2.6);
   box(room,dark,r.minX+.5,5.57,cz-.6,.6,.035,.65);
   for(const z of [cz-.82,cz-.4]){const hob=new THREE.Mesh(new THREE.TorusGeometry(.12,.022,6,16),brass);hob.rotation.x=Math.PI/2;hob.position.set(r.minX+.5,5.6,z);room.add(hob);}
   box(room,brass,r.minX+.5,5.56,cz+.65,.65,.04,.6);
  }
  ...
  const breakfastX=cx+(home.floorY===0?3:.3);
  g=item('breakfast');box(g,wood,breakfastX,5.1,r.maxZ-1.1,1.15,.09,.85);box(g,brass,breakfastX,4.8,r.maxZ-1.1,.1,.6,.1);box(g,white,breakfastX+.2,5.25,r.maxZ-1.1,.16,.22,.16);box(g,cream,breakfastX-.28,5.17,r.maxZ-1.1,.4,.035,.35);
  decor.set(home.id,items);
 }
 // Street listings board is backed and mounted on two posts, outside the entry halls.
 box(root,wood,72.5,1.6,30.8,2.2,1.4,.16);
 for(const x of [71.6,73.4])box(root,brass,x,.65,30.8,.08,1.3,.08);
 sign(root,['MERCER ROW · HOMES FOR SALE','GARDEN FLAT $260 · PARKVIEW $420'],72.5,1.6,30.71,2.05,1.25,Math.PI);
 ...
 return {root,doors,decor,setState(input){const s=normalizeLife(input);doors.forEach((d,id)=>d.visible=!s.properties.includes(id));
  for(const b of colliders.boxes)if(b.homeGate){const owned=s.properties.includes(b.homeGate);const floor=HOMES.find(h=>h.id===b.homeGate)?.floorY??4.4;b.minY=owned?-100:floor;b.maxY=owned?-99:floor+3.3;}
