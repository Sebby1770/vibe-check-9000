...
export function animateHuman(obj, t, ctx = {}) {
    ...
    if (mode === "walk") {
        ...
        j.lThigh.rotation.x = s * 0.7;
        j.rThigh.rotation.x = -s * 0.7;
        j.lShin.rotation.x = Math.max(0, -s) * 0.55;
        j.rShin.rotation.x = Math.max(0, s) * 0.55;
        j.lUpper.rotation.x = -s * 0.45;
        j.rUpper.rotation.x = s * 0.45;
        j.spine.rotation.y = s * 0.06;
        j.hips.position.y = 0.95 + Math.abs(s) * 0.02;
        j.head.rotation.y = -s * 0.04;
    } else if (mode === "dance") {
        const b = Math.abs(Math.sin(beat));
        j.hips.position.y = 0.95 + b * 0.07;
        j.hips.rotation.y = Math.sin(beat * 0.5) * 0.18;
        j.lUpper.rotation.z = 0.9 + Math.sin(beat) * 0.35;
        j.rUpper.rotation.z = -0.9 - Math.sin(beat + 1) * 0.35;
        j.lUpper.rotation.x = -0.4 + Math.sin(beat * 2) * 0.25;
        ...
        j.spine.rotation.y = Math.sin(t * 0.35 + ph) * 0.08;
        j.head.rotation.y = Math.sin(t * 0.25 + ph) * 0.12;
        j.head.rotation.x = Math.sin(t * 0.5 + ph) * 0.03;
        j.lUpper.rotation.z = 0.12 + breath;
        j.rUpper.rotation.z = -0.12 - breath;
        j.lUpper.rotation.x = 0.05;
        j.rUpper.rotation.x = 0.08;
        j.lThigh.rotation.x = 0.04;
        j.rThigh.rotation.x = -0.03;
        j.lShin.rotation.x = 0.04;
        j.rShin.rotation.x = 0.02;
        if (ctx.talking) {
            j.rFore.rotation.x = -0.4 + Math.sin(t * 6) * 0.2;
            j.rUpper.rotation.x = -0.3;
        }
    }
}
